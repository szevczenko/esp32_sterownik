...
static void _state_working( void )
{
  ...
  if ( menuGetValue( MENU_CURRENT_MOTOR ) > 100 && menuGetValue( MENU_MOTOR_IS_ON ) /*&& menuGetValue(MENU_ERROR_MOTOR) */ )
  {
    _change_state( STATE_ERROR_MOTOR_CURRENT );
  }

  if ( _is_overcurrent() && menuGetValue( MENU_ERROR_MOTOR ) && menuGetValue( MENU_MOTOR_IS_ON ) )
  {
  ...
  if ( ( temperature > 80 && menuGetValue( MENU_ERROR_MOTOR ) ) || ( temperature > 90 ) )
  {
